...
require('go').setup({
  ...
  iferr_vertical_shift = 4 -- defines where the cursor will end up vertically from the begining of if err statement
     ...
     -- Function to set keymaps for Go formatting
  local function set_go_keymaps()
    local opts = { noremap = true, silent = true }
    vim.api.nvim_set_keymap("n", "<leader>gf", ":GoFmt<CR>", opts) -- Keymap for gofumpt
    vim.api.nvim_set_keymap("n", "<leader>gl", ":GoLines<CR>", opts) -- Keymap for golines
      end
  end,
  event = {"CmdlineEnter"},
  ft = {"go", 'gomod'},
